...
class AudioChunkCollectorNode extends AudioWorkletProcessor {
    ...
    process(inputs, outputs) {
        if (this._stopProcessing) {
            this.port.postMessage({ message: 'audioData', audioData: this._chunkBuffer }) // Letzten Chunk, auch wenn unvollständig, zurück schicken
            this._currentChunkIndex = 0 // Für den nächsten Lauf zurücksetzen
            return false
        }
            ...
            if (firstInput.length > 0) {
                ...
                for (let i = 0; i < firstInputChannel.length; i++, this._currentChunkIndex++) {
                    ...
                    if (this._currentChunkIndex >= this._chunkBufferSize - 1) {
                        ...
                        this._chunkBuffer.fill(0) // Puffer leeren
